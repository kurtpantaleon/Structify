...
function SubHeading2({ progress = 1, totalSteps = 8, onNext, onPrev, exitPath = '/mainPage' }) {
  ...
  return (
    <div className="bg-[#1F274D] flex items-center justify-between px-4 sm:px-7 py-3 shadow-md border-b border-white/10">
      {/* Exit or menu button */}
      <button onClick={() => navigate(exitPath)} className="transition-transform transform hover:scale-110">
        <img src={XIcon} alt="Exit Icon" className="h-6 w-6 sm:h-5 sm:w-5 cursor-pointer" />
      </button>
      ...
      <div className="flex items-center space-x-4 sm:space-x-10">
        ...
        <div className="flex items-center space-x-1 sm:space-x-2">
          {Array.from({ length: totalSteps }).map((_, index) => (
            <div
              key={index}
              className={`h-1.5 w-6 sm:w-14 rounded-full transition-all duration-300 ${index < progress ? 'bg-green-400' : 'bg-gray-600'}`}
            ></div>
          ))}
        </div>
      </div>
