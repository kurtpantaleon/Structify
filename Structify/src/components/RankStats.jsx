...
const styles = {
  container:
    "w-full max-w-2xl mx-auto text-white rounded-2xl shadow-2xl overflow-hidden backdrop-blur-sm bg-opacity-95",
  header:
    "flex justify-center items-center gap-6 p-6 w-full rounded-t-lg relative overflow-hidden",
  profileImage:
    "w-32 h-32 rounded-full border-4 border-white/80 shadow-lg shadow-blue-700/30 object-cover transition-all duration-300 hover:scale-105",
  rankDisplay:
    "flex justify-center items-center py-2 text-black bg-no-repeat bg-center transition-transform duration-300 hover:scale-105",
  scoreCard:
    "relative w-full sm:w-5/6 flex justify-between items-center py-3 px-5 rounded-xl text-white overflow-hidden bg-center bg-no-repeat bg-cover ring-2 ring-blue-400/50 shadow-xl hover:ring-blue-400 transition-all duration-300",
  shineEffect:
    "absolute top-0 left-[-75%] w-[80%] h-full bg-white/20 rotate-[25deg] animate-shine",
  challengeButton:
    "w-full sm:w-auto py-3 px-8 rounded-xl text-white font-semibold flex items-center justify-center gap-x-3 shadow-lg shadow-blue-600/40 hover:shadow-blue-600/70 transition duration-300 ease-in-out hover:scale-105 bg-no-repeat transform active:scale-95 active:shadow-inner",
};
...
const calculateProgress = (
  ...
  const completed =
    expectedActivities.filter((a) => completedActivities.includes(a)).length +
    ...
    const fetchBattleRecords = async () => {
      ...
      try {
        ...
        try {
          ...
          console.log(
            "Fetched matches using simple queries:",
            allMatches.length
          );
        } catch (error) {
          console.error(
            "Error with simple query, falling back to sample data:",
            error
          );
        ...
        const opponentPromises = allMatches.map(async (match) => {
          try {
            ...
            return {
              ...match,
              opponentName: opponentSnap.exists()
                ? opponentSnap.data().name
                : "Unknown Player",
              timeAgo: calculateTimeAgo(match.timestamp?.toDate()),
            };
          } catch (error) {
        ...
        <div className="flex flex-col flex-1 z-10">
          <div>
            <div className="flex items-end gap-2">
              ...
              <div className="mb-1 px-2 py-0.5 bg-blue-700/50 rounded-lg text-xs font-medium">
                Student
              </div>
            </div>
            ...
            <div className="flex items-center w-full mt-2 mb-4">
              <div className="flex-1 bg-gray-800/50 h-3 rounded-full relative overflow-hidden ring-1 ring-white/10">
                ...
                {progress > 15 && (
                  <div
                    ...
                    style={{
                      left: `${Math.min(Math.max(progress - 5, 5), 90)}%`,
                    }}
                  >
                    {progress}%
                  </div>
              ...
              {progress <= 15 && (
                <div className="ml-2 text-xs text-white/90 w-10 text-right font-medium">
                  {progress}%
                </div>
              )}
            </div>
        ...
        {battleLoading ? (
          <div className="flex flex-col gap-3 items-center">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="w-5/6 h-16 bg-gray-800/50 rounded-xl animate-pulse"
              ></div>
            ))}
          </div>
        ) : battleError ? (
          <div className="text-center py-6 text-red-300">
            {typeof battleError === "string" ? (
              <p>{battleError}</p>
            ) : (
              battleError
            )}
          ...
          <div className="text-center py-8 text-gray-400 border border-dashed border-gray-700 rounded-xl">
            ...
            <p className="text-sm">
              Challenge another student to start competing
            </p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-4">
            {matches.map((battle, index) => {
              const userScore = battle.isPlayer1
                ? battle.score1
                : battle.score2;
              const opponentScore = battle.isPlayer1
                ? battle.score2
                : battle.score1;
                ...
                >
                  ...
                  <div className="z-10 text-left flex flex-col">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="font-bold text-lg text-blue-200 drop-shadow">
                        You
                      </p>
                    </div>
                  ...
                  <div className="z-10 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
                    <div className="text-sm font-bold bg-indigo-900/70 px-2 py-1 rounded-lg mb-1 text-blue-200">
                      VS
                    </div>
                  ...
                  <div className="z-10 text-right flex flex-col items-end">
                    <div className="flex items-center gap-2 justify-end">
                      <p className="font-bold text-lg text-red-200 drop-shadow">
                        {battle.opponentName}
                      </p>
