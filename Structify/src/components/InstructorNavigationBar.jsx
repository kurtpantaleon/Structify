...
const InstructorNavigationBar = () => {
  return (
    <div className="fixed bg-[#1F274D] h-screen w-22 flex flex-col items-center py-8 px-1 border-gray-200">
      {/* Navigation Icons */}
      <nav className="flex flex-col space-y-9 gap-y-3">
        ...
        <NavItem to="/AddLessonMaterials" icon={AddIcon} label="Upload Class Materials" />
...
const NavItem = ({ to, icon, label }) => {
  ...
  return (
    <div className="flex flex-col items-center cursor-pointer group" onClick={handleNavigation}>
      ...
      <span className="text-white text-xs mt-1 text-center w-full">{label}</span>
    </div>
  );
};
