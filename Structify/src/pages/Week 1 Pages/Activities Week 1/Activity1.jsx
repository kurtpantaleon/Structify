...
export default function Activity1() {
  ...
  const handleEditorDidMount = (editor, monaco) => {
    ...
    // Optionally, you can dynamically get all Monaco languages here
  ...
  const runCode = () => {
    ...
    if (language === "javascript") {
      ...
      setConsoleOutput(
        "Code execution for this language is not supported in-browser. Please visually check your output."
      );
      setFeedback(
      ...
      <div className="flex-1 flex flex-col min-h-0 p-4 items-center">
        ...
        <div className="w-full max-w-2xl flex-1 flex flex-col min-h-0">
          ...
          <div className="mt-4 bg-[#1A2A4B] p-3 rounded-lg text-white font-mono min-h-[40px]">
            ...
            {feedback && (
              <div className="mt-2 font-bold text-cyan-300">{feedback}</div>
            )}
          </div>
        </div>
      </div>
